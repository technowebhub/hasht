                ...
                if ($tweet->id > $twitter['last_id'] && (!empty($tweet->entities->urls) || isset($tweet->entities->media))){
                    ...
                    if ($is_tweet || $is_vine){
                        if (mysqli_query($db_con,
                            ...
                            "values('$time_now', '$twitter_id', '$created_at','$user_id','$this_name','$screen_name', '$user_location', '$text', '$media_url', '$media_url_https', 'twitter', '$type', '$hashtag', '$link_post')")or die(mysqli_connect_error())){}
...
function outputFeed($db, $hashtag){
    ...
    if (mysqli_num_rows($query) > 0) {
        while ($post = mysqli_fetch_assoc($query)) {
            ...
            $html .= '<li class="'.$post['source'].' col-sm-12">'.$media.'</li>';
        }
    }
    $html .= '</ul>';
    mysqli_close($db_con);
